...
class ReporterProcessState(ShmWrapperForReporter):
    def __del__(self):
        ...
        write(self.shm_fd, pack('b', 0))
        lseek(self.shm_fd, self.offset + 2, SEEK_SET)
        write(self.shm_fd, pack('b', 0))
        lseek(self.shm_fd, self.offset + 3, SEEK_SET)
        write(self.shm_fd, pack('b', 0))
        lseek(self.shm_fd, self.offset + 4, SEEK_SET)
        write(self.shm_fd, pack('b', 0))
    ...
    def on_log_process_state(indyshm):
        lseek(indyshm.shm_fd, indyshm.offset + 2, SEEK_SET)
        return write(indyshm.shm_fd, pack('b', 1))

    @staticmethod
    def on_zip_process_state(indyshm):
        lseek(indyshm.shm_fd, indyshm.offset + 3, SEEK_SET)
        return write(indyshm.shm_fd, pack('b', 1))

    @staticmethod
    def on_serial_number_register_state(indyshm):
        lseek(indyshm.shm_fd, indyshm.offset + 4, SEEK_SET)
        return write(indyshm.shm_fd, pack('b', 1))
    ...
    def write_serial_number(indyshm, _str):
        lseek(indyshm.shm_fd, indyshm.offset + 5, SEEK_SET)
        return write(indyshm.shm_fd, pack('%ss' % len(_str.encode()), _str.encode()))
